...
function Header()
{
	...
	$this->SetFont("", "B", 10);
	if($_GET['idsub'] == 'TQ'){
    $this->SetX($left); $this->Cell(0, 10, 'LAPORAN DATA MASUK BARANG TEST QUALITY', 0, 1,'C');
    }else if ($_GET['idsub'] == 'PACKING'){
    $this->SetX($left); $this->Cell(0, 10, 'LAPORAN DATA MASUK BARANG PACKING', 0, 1,'C');
    }else if ($_GET['idsub'] == 'ATK'){
    $this->SetX($left); $this->Cell(0, 10, 'LAPORAN DATA MASUK BARANG ATK', 0, 1,'C');
    }
     $this->settitle('Stock Barang Masuk');   
        $this->SetFillColor(200,200,200);
        $this->SetFont("", "B", 8);
